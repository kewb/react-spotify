...
const refreshAccessToken = async () => {
  try {
    ...
    // e.g., redirect to login, display error message, etc.
  }
};

// Check if the token is valid (consolidated logic)
const isTokenValid = () => {
  const tokenTimestamp = getTokenTimestamp();
  return tokenTimestamp && Date.now() - tokenTimestamp < EXPIRATION_TIME;
}

// Get access token (main logic)
export const getAccessToken = () => {
  ...
  if (!isTokenValid()) {
    ...
    return refreshAccessToken();  // Return the promise from refreshAccessToken
  }
...
export const token = getAccessToken(); // Get token on module load
 ...
 */
export const getMyCurrentPlayingTrack = () => axios.get('https://api.spotify.com/v1/me/player/currently-playing', { headers });
